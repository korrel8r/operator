...
type requestContext struct {
	*Korrel8rReconciler // Parent reconciler
	...
	// Context and values computed per-reconcile
	ctx context.Context
...
func (r *requestContext) updateKorrel8r() error {
	// Fill in default configuration if needed
	if r.Korrel8r.Spec.Config == nil {
		r.Korrel8r.Spec.Config = &korrel8rv1alpha1.Config{
		...
		r.logChange(controllerutil.OperationResultUpdated, nil, &r.Korrel8r)
	}
	return nil
}
...
func (r *requestContext) configMap() error {
	...
	return r.createOrUpdate(cm, func() error {
		...
		if string(conf) != cm.Data[ConfigKey] {
	...
	return r.createOrUpdate(d, func() error {
		...
		if r.Korrel8r.Spec.Verbose > 0 { //  Set KORREL8R_VERBOSE environment variable
			d.Spec.Template.Spec.Containers[0].Env = []corev1.EnvVar{
				{
					Name:  VerboseEnv,
					Value: strconv.Itoa(r.Korrel8r.Spec.Verbose),
				},
			}
		}
