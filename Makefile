...
.PHONY: manifests
manifests: controller-gen kustomize $(MAKEFILES) ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=manager-role crd webhook paths="./..." output:crd:artifacts:config=config/crd/bases
	cd config/manager && $(KUSTOMIZE) edit set image controller=$(IMAGE)

ICON_YAML=config/manifests/patch_csv_icon.yaml
$(ICON_YAML): hack/icon.png
	hack/icon_yaml.sh < $^ > $@

.PHONY: generate
generate: controller-gen $(ICON_YAML) ## Generate code containing DeepCopy, DeepCopyInto, and DeepCopyObject methods.
